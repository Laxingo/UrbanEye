...
async function runDeleteEventTest() {
  ...
  try {
    ...
    let cards = await driver.findElements(By.css(".event-card"));
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      if (creatorText.includes(CURRENT_USER_EMAIL)) {
        ...
        console.log("🗑 A eliminar evento...");

        // Se existir confirmação, trata aqui
        const confirmButtons = await driver.findElements(By.xpath("//button[contains(text(),'Confirm')]"));
    ...
    if (!deleted) {
      console.log("❌ Nenhum evento criado pelo utilizador foi encontrado para eliminar.");
      return;
    }

    // Validar que o evento desapareceu da lista
    await sleep(800);

    const updatedCards = await driver.findElements(By.css(".event-card"));
    console.log(`Eventos após eliminação: ${updatedCards.length}`);

    if (updatedCards.length < cards.length) {
      console.log("🎉 Evento eliminado com sucesso!");
    } else {
      console.log("⚠ O evento não desapareceu da lista (verificar backend).");
    }
